...
def eval_3DMatch(model, config, use_icp):
    ...
    logging.info(f"*" * 40)
    content = f"All {allpair_stats.shape[0]} pairs, Mean Reg Recall={allpair_average[0] * 100:.2f}%, Mean Re={correct_pair_average[1]:.2f}, Mean Te={correct_pair_average[2]:.2f}"
    ...
    return all_stats_npy,content,allpair_average[0] * 100
...
if __name__ == '__main__':
    ...
    import glob
    '''
        model_59_recall_92.02898550724638.pkl
        The Best recall:93.53049907578558,
    '''
    best_recall = -1
    best_checkpoint = ""
    files_path = "/DISK/qwt/PointDSC_Ours/cross_posi_FPFH/logs/logs.txt"
    models_path = "/DISK/qwt/PointDSC_Ours/cross_posi_FPFH/snapshot/PointDSC_3DMatch_01302115/models"
    checkpoint_paths = glob.glob(os.path.join(models_path,"model_*.pkl"))
    with open(file=files_path,mode="w") as f:

        for i,checkpoint_path in enumerate(checkpoint_paths):

            print(f"--- current checkpoint index:{i+1} ---")

            snapshot_path = "PointDSC_Ours"
            logs_path = "/DISK/qwt/PointDSC_Ours/logs"
            config_path = f'/DISK/qwt/PointDSC_Ours/cross_posi_FPFH/snapshot/PointDSC_3DMatch_01302115/config.json'

            # checkpoint_path = "/DISK/qwt/PointDSC_Ours/cross/snapshot/PointDSC_3DMatch_12242049/models/model_27_recall_92.7536231884058.pkl"
            ...
            stats,content,recall = eval_3DMatch(model.cuda(), config, args.use_icp)
            ...
            # write the result
            content = f"{checkpoint_path} --- {content}\n"
            f.write(content)
            print("\nThe result has wrote in the log!")
            print(f"Current Best recall:{best_recall}, checkpoint_path:{best_checkpoint}")
    print(f"The Best recall:{best_recall}, checkpoint_path:{best_checkpoint}")
    pass