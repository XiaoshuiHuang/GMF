...
# FCGF feats
def process_3dmatch_train(voxel_size=0.05,model=None):
    ...
    save_path = "/DISK/qwt/pointDSC/FCGF_feat_train"
    # scene
    ...
    for scene in scenes:
        ...
        for seq in seqs:
            ...
            for ply in plys:
                ...
                ply_save_path = os.path.join(folder_path,f"{ply}_fcgf.npz")
                np.savez_compressed(
                    ply_save_path,
...
def process_3dmatch_test(voxel_size=0.05):
    ...
    save_path = "/DISK/qwt/pointDSC/FCGF_feat_test"
    # scene
    # scenes = [scene.split("/")[-1] for scene in os.listdir(root)]
    for scene in scenes:
        seqs_path = os.path.join(root,scene,"seq-*")
...
if __name__ == '__main__':
    ...
    check_path= "/home/qwt/code/PointDSC-master/misc/ResUNetBN2C-feat32-3dmatch-v0.05.pth"
    checkpoint = torch.load(check_path)
    model.load_state_dict(checkpoint['state_dict'])
    model.eval()

    process_3dmatch_train(voxel_size=0.05,model=model)
