...
# 根据权重通过最小二乘拟合求A和B的刚性变换
def rigid_transform_3d(A, B, weights=None, weight_threshold=0):
    """ 
    Input:
        - A:       [B*num_seeds,k,3], source point cloud
        - B:       [B*num_seeds,k,3], target point cloud
        - weights: [B*num_seeds,k]    weight for each correspondence
        - weight_threshold: float,    clips points with weight below threshold
    Output:
        - R, t ,R : [B*num_seeds,3,3],t : [B*num_seeds,3,1]
        - T : [B*num_seeds,4,4]
    """
    bs = A.shape[0]
    if weights is None:
...
def knn(x, k, ignore_self=False, normalized=True):
    ...
    Input:
        ...
        - k:       
        - ignore_self:  True/False, 范围的KNN邻居是否包含自身
        - normalized:   True/False, 是否对特征进行归一化
    Output:
        - idx:     [bs, num_corr, k], 每个x的前k个邻居的索引
    """
