...
class NonLocalBlock(nn.Module):
    def __init__(self, num_channels=128, num_heads=1):
        ...
        self.fusion_layer_2 = FusionLayer(
            dim=num_channels,  # the image channels
            depth=0,  # depth of net (self-attention)
            latent_dim=num_channels,  # the PC channels
            cross_heads=1,  # number of heads for cross attention. paper said 1
            latent_heads=8,  # number of heads for latent self attention, 8
            cross_dim_head=int(num_channels//2),  # number of dimensions per cross attention head
            latent_dim_head=int(num_channels//2),  # number of dimensions per latent self attention head
            pe=True
        )
    ...
    def forward(
            ...
            feat,
            attention,
            image_feat,                  # correspondence image feat
    ):
        """
        Input:
        ...
        # [B,1,128,1000],[B,1,128,1000] -> [B,1,1000,1000]
        ...
        # image_feat
        image_feat = self.fusion_layer_2(
            image_feat,
            queries_encoder=feat.permute(0,2,1),
        ).permute(0,2,1)

        res = message + image_feat
        return res 
...
class NonLocalNet(nn.Module):
    def __init__(
        ...
        self.fusion_layer_1 = FusionLayer(
            dim=num_channels,  # the image channels
            depth=0,  # depth of net (self-attention)
            latent_dim=num_channels,  # the PC channels
            cross_heads=1,  # number of heads for cross attention. paper said 1
    ...
    def forward(
        ...
        p_image = self.image_encoder(p_image)
        B, C, H, W = p_image.shape
        p_image = p_image.view(B, C, H * W).permute(0, 2, 1)

        q_image = self.image_encoder(q_image)
        B, C, H, W = q_image.shape
        q_image = q_image.view(B, C, H * W).permute(0, 2, 1)

        image_feat = self.fusion_layer_1(p_image, queries_encoder=q_image)

        feat = self.layer0(corr_feat)
        for i in range(self.num_layers):
            feat = self.blocks[f'PointCN_layer_{i}'](feat)
            feat = self.blocks[f'NonLocal_layer_{i}'](feat, corr_compatibility,image_feat)
        return feat
...
class PointDSC(nn.Module):
    def __init__(
        ...
        self.encoder = NonLocalNet(
            in_dim=in_dim,                      # 6，(x1,y1,z1,x2,y2,z2)
    ...
    def forward(self, data):
        ...
        p_image,q_image = data["p_image"],data["q_image"]
        ...
        #################################
        # Step 1:（correspondences length and feature similarity）
        #################################
        with torch.no_grad():
        ...
        corr_features = self.encoder(
            corr_pos.permute(0,2,1),
            corr_compatibility,
            p_image,
            q_image
        ).permute(0, 2, 1)
    ...
    def cal_leading_eigenvector(self, M, method='power'):
        ...
        Input: 
            ...
            - method: select different method for calculating the leading eigenvector.
        Output: 
