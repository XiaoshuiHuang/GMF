...
from model.perceiver_io import PerceiverIO
  ...
  def forward(self, x):
    ...
    if self.normalize_feature:
      return ME.SparseTensor(
          ...
          coords_key=out.coordinate_map_key,
          coords_manager=out.coordinate_manager)
    else:
      return out
...
class ResUNet2(ME.MinkowskiNetwork):
  ...
  def __init__(self,
    ...
    self.image_fusion = PerceiverIO(
        dim=128,  # the image channels
        depth=0,  # depth of net (self-attention - Processing的数量)
        latent_dim=128,  # the PC channels
        cross_heads=1,  # number of heads for cross attention. paper said 1
        latent_heads=8,  # number of heads for latent self attention, 8
  ...
  def forward(self, x,p_image,q_image):

    p_image = self.img_encoder(p_image)
    B, C, H, W = p_image.shape
    p_image = p_image.view(B, C, H * W).permute(0, 2, 1)

    q_image = self.img_encoder(q_image)
    B, C, H, W = q_image.shape
    q_image = q_image.view(B, C, H * W).permute(0, 2, 1)

    image_feat = self.image_fusion(p_image, queries_encoder=q_image)
    ...
    # out._F = self.transformer(image=q_image, F=out.F)
    out._F = self.transformer(image=image_feat, F=out.F)
  ...
  def transformer(self, image, F):

    P_att = torch.unsqueeze(F, dim=0)
    # B, C, H, W = image.shape
    # image = image.view(B, C, H * W)
    # image = image.permute(0, 2, 1)  # [B,H*W,C]
    P_att = self.perceiver_io(image, queries_encoder=P_att)
    P_att = torch.squeeze(P_att)
    # F = torch.cat([F,P_att],dim=-1)
    F = P_att
