...
class CollationFunctionFactory:
  ...
  def collate_pair_fn(self, list_data):
    ...
    xyz0, xyz1, coords0, coords1, feats0, feats1, matching_inds, trans, extra_packages,p_image,q_image = list(
        zip(*list_data))
    # xyz0_batch = []
    # xyz1_batch = []
    # for index, xyz_0,xyz_1 in zip(xyz0,xyz1):
    #     xyz0_batch.append(xyz_0)
    #     xyz1_batch.append(xyz_1)

    matching_inds_batch, trans_batch, len_batch = [], [], []

    xyz0_batch =  np.concatenate(xyz0,axis=0)
    xyz1_batch =  np.concatenate(xyz1,axis=0)
    ...
    p_image = torch.from_numpy(p_image)
    q_image = q_image[0]
    q_image = np.expand_dims(q_image, axis=0)
    q_image = torch.from_numpy(q_image)
    ...
    return {
        'pcd0': xyz0,
        'pcd1': xyz1,
        'pcd0_batch': xyz0_batch,
        'pcd1_batch': xyz1_batch,
