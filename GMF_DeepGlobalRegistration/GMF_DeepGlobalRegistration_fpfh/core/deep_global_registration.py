...
def registration_ransac_based_on_correspondence(pcd0, pcd1, idx0, idx1,
  ...
  result = o3d.registration.registration_ransac_based_on_correspondence(
      ...
      ransac_n=4,
      criteria=o3d.registration.RANSACConvergenceCriteria(4000000, num_iterations))
...
class DeepGlobalRegistration:
  def __init__(self, config, device=torch.device('cuda')):
    ...
    InlierModel = load_model(network_config['inlier_model'],True)
  ...
  def fcgf_feature_extraction(self, feats, coords, xyz=None):
    '''
    Step 1: extract fast and accurate FCGF feature per point
    '''
    # sinput = ME.SparseTensor(feats, coords=coords).to(self.device)
    if(xyz is not None):
        i0_xyz = make_open3d_point_cloud(xyz)
        i0_xyz.estimate_normals(
            search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.05 * 2, max_nn=30))
        ...
        oF0 = np.array(fpfh.data).T
        np.nan_to_num(oF0)
        oF0 = oF0 / (np.linalg.norm(oF0, axis=1, keepdims=True) + 1e-6)
        oF0 = torch.from_numpy(oF0).cuda()
        return oF0
    else:
  ...
  def inlier_feature_generation(self, xyz0, xyz1, coords0, coords1, fcgf_feats0,
                                fcgf_feats1, corres_idx0, corres_idx1,p_image=None,q_image=None):
    '''
  ...
  def inlier_prediction(self, inlier_feats, coords,p_image=None,q_image=None):
    '''
    Step 4: predict inlier likelihood
    '''
    # sinput = ME.SparseTensor(inlier_feats, coords=coords).to(self.device)
    sinput = ME.SparseTensor(
        inlier_feats.to(self.device),
        coordinates=coords.to(self.device))
    p_image = p_image.to(self.device)
    q_image = q_image.to(self.device)
    soutput = self.inlier_model(sinput,p_image,q_image)
  ...
  def register(self, xyz0, xyz1, inlier_thr=0.00,p_image=None,q_image=None,use_corr=False):
    '''
    ...
    with torch.no_grad():
      ...
      p_image = torch.from_numpy(np.expand_dims(p_image,axis=0))
      q_image = torch.from_numpy(np.expand_dims(q_image,axis=0))

      # Step 1: Feature extraction
      self.feat_timer.tic()
      fcgf_feats0 = self.fcgf_feature_extraction(feats0, coords0,xyz=xyz0)
      fcgf_feats1 = self.fcgf_feature_extraction(feats1, coords1,xyz=xyz1)
      self.feat_timer.toc()
      ...
      inlier_feats = self.inlier_feature_generation(xyz0, xyz1, coords0, coords1,
                                                    ...
                                                    corres_idx0, corres_idx1,p_image,q_image)

      # Step 4: Inlier likelihood estimation and truncation
      logit = self.inlier_prediction(inlier_feats.contiguous(), coords=inlier_coords,p_image=p_image,q_image=q_image)
    ...
    if (use_corr):
        return T, xyz0[corres_idx0], xyz1[corres_idx1]
    else:
        return T
