...
class WeightedProcrustesTrainer:
  def __init__(self, config, data_loader, val_data_loader=None):
    ...
    InlierModel = load_model(config.inlier_model,True)
  ...
  def train(self):
    ...
    for epoch in range(self.start_epoch, self.max_epoch + 1):
      ...
      if self.test_valid and epoch % self.val_epoch_freq == 0:
        ...
        self._save_checkpoint(epoch,recall=val_dict[self.best_val_metric])
    ...
    for curr_iter in range(num_train_iter):
      ...
      for iter_idx in range(iter_size):
        ...
        reg_coords, reg_feats, pred_pairs, is_correct, feat_time, nn_time = self.generate_inlier_input(
            ...
            i0_xyz=input_dict['pcd0_batch'],
            i1_xyz=input_dict['pcd1_batch'],
        )
        ...
        reg_sinput = ME.SparseTensor(
            reg_feats.contiguous().to(self.device),
            coordinates=reg_coords.int().to(self.device))
        p_image = input_dict["p_image"].to(self.device)
        q_image = input_dict["q_image"].to(self.device)
        # reg_sinput = ME.SparseTensor(reg_feats.contiguous(),
        #                              coords=reg_coords.int()).to(self.device)
        reg_soutput = self.inlier_model(reg_sinput,p_image,q_image)
        inlier_timer.toc()
  ...
  def _valid_epoch(self):
    ...
    for batch_idx in range(tot_num_data):
      ...
      q_image = input_dict["q_image"].to(self.device)
      # reg_sinput = ME.SparseTensor(
      #     reg_feats.contiguous(),
      #   coords=reg_coords.int()
      # ).to(self.device)
      reg_soutput = self.inlier_model(reg_sinput,p_image,q_image)
      inlier_timer.toc()
  ...
  def _save_checkpoint(self, epoch, filename='checkpoint',recall=None):
    """
    Saving checkpoints
    ...
    filename = os.path.join(self.checkpoint_dir, f'{filename}_{epoch}_{recall}.pth')
  ...
  def generate_inlier_input(self, xyz0, xyz1, iC0, iC1, iF0, iF1, len_batch, pos_pairs,i0_xyz=None,i1_xyz=None):
    ...
    i0_xyz = make_open3d_point_cloud(i0_xyz)
    ...
    #     iF0.to(self.device),
    #     coordinates=iC0.to(self.device))
    # oF0 = self.feat_model(sinput0).F

    # sinput1 = ME.SparseTensor(iF1, coords=iC1).to(self.device)
    i1_xyz = make_open3d_point_cloud(i1_xyz)
    i1_xyz.estimate_normals(
        search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.05 * 2, max_nn=30))
    fpfh = o3d.registration.compute_fpfh_feature(
        i1_xyz,
        o3d.geometry.KDTreeSearchParamHybrid(
    ...
    oF1 = np.array(fpfh.data).T
    np.nan_to_num(oF1)
    oF1 = oF1 / (np.linalg.norm(oF1, axis=1, keepdims=True) + 1e-6)
    oF1 = torch.from_numpy(oF1).cuda()
    # sinput1 = ME.SparseTensor(
    #     iF1.to(self.device),
    #     coordinates=iC1.to(self.device))
